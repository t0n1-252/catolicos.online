...
try {
  ...
  switch ($_GET['view']) {
    case '':
      // page header
      page_header(__("Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));
      break;

    case 'users':
      // page header
      page_header(__("Users Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM users WHERE users.user_banned = '0'");
    ...
    case 'posts':
      // page header
      page_header(__("Posts Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM posts LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user' LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page' LEFT JOIN `groups` ON posts.in_group = '1' AND posts.group_id = `groups`.group_id LEFT JOIN `events` ON posts.in_event = '1' AND posts.event_id = `events`.event_id WHERE posts.privacy = 'public' AND (posts.pre_approved = '1' OR posts.has_approved = '1') AND NOT (users.user_name <=> NULL AND pages.page_name <=> NULL)");
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      ...
      // get posts
      $rows = [];
      $get_rows = $db->query("SELECT posts.post_id FROM posts LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user' LEFT JOIN pages ON posts.user_id = pages.page_id AND posts.user_type = 'page' LEFT JOIN `groups` ON posts.in_group = '1' AND posts.group_id = `groups`.group_id LEFT JOIN `events` ON posts.in_event = '1' AND posts.event_id = `events`.event_id WHERE posts.privacy = 'public' AND (posts.pre_approved = '1' OR posts.has_approved = '1') AND NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) " . $limit_query);
      while ($row = $get_rows->fetch_assoc()) {
    ...
    case 'pages':
      ...
      // page header
      page_header(__("Pages Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM pages");
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      ...
      // get pages
      $rows = [];
      $get_rows = $db->query("SELECT * FROM pages " . $limit_query);
      while ($row = $get_rows->fetch_assoc()) {
        $row['page_picture'] = get_picture($row['page_picture'], 'page');
        /* check if the viewer liked the page */
        $row['i_like'] = false;
        if ($user->_logged_in) {
          $get_likes = $db->query(sprintf("SELECT * FROM pages_likes WHERE page_id = %s AND user_id = %s", secure($row['page_id'], 'int'), secure($user->_data['user_id'], 'int')));
    ...
    case 'groups':
      ...
      // page header
      page_header(__("Groups Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM `groups` WHERE group_privacy = 'public'");
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      ...
      // get groups
      $rows = [];
      $get_rows = $db->query("SELECT * FROM `groups` WHERE group_privacy = 'public' " . $limit_query);
      while ($row = $get_rows->fetch_assoc()) {
    ...
    case 'events':
      ...
      // page header
      page_header(__("Events Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM `events` WHERE event_privacy = 'public'");
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      ...
      // get events
      $rows = [];
      $get_rows = $db->query("SELECT * FROM `events` WHERE event_privacy = 'public' " . $limit_query);
      while ($row = $get_rows->fetch_assoc()) {
    ...
    case 'games':
      ...
      // page header
      page_header(__("Games Directory") . ' | ' . __($system['system_title']), __($system['system_description_directory']));

      // pager config
      require('includes/class-pager.php');
      $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
      $total = $db->query("SELECT COUNT(*) as count FROM games");
      $params['total_items'] = $total->fetch_assoc()['count'];
      $params['items_per_page'] = $system['max_results'];
      ...
      // get games
      $rows = [];
      $get_rows = $db->query("SELECT * FROM games " . $limit_query);
      while ($row = $get_rows->fetch_assoc()) {
