...
try {
  ...
  switch ($_GET['do']) {
    case 'edit':
      ...
      if (!in_array($_POST['field_for'], ['user', 'page', 'group', 'event', 'product', 'job', 'offer', 'course'])) {
      ...
      $_POST['in_registration'] = (isset($_POST['in_registration']) || in_array($_POST['field_for'], ['product', 'job', 'offer', 'course'])) ? '1' : '0';
      $_POST['in_profile'] = (isset($_POST['in_profile']) || in_array($_POST['field_for'], ['product', 'job', 'offer', 'course'])) ? '1' : '0';
      $_POST['in_search'] = (!isset($_POST['in_search']) || in_array($_POST['field_for'], ['product', 'job', 'offer', 'course'])) ? '0' : '1';
      /* update */
      $db->query(sprintf("UPDATE custom_fields SET field_for = %s, type = %s, select_options = %s, label = %s, description = %s, place = %s, length = %s, field_order = %s, is_link = %s, mandatory = %s, in_registration = %s, in_profile = %s, in_search = %s WHERE field_id = %s", secure($_POST['field_for']), secure($_POST['type']), secure($_POST['select_options']), secure($_POST['label']), secure($_POST['description']), secure($_POST['place']), secure($_POST['length'], 'int'), secure($_POST['field_order'], 'int'), secure($_POST['is_link']), secure($_POST['mandatory']), secure($_POST['in_registration']), secure($_POST['in_profile']), secure($_POST['in_search']), secure($_GET['id'], 'int')));
      /* return */
      return_json(['success' => true, 'message' => __("Field info have been updated")]);
      break;
