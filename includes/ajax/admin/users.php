...
try {
  ...
  // valid inputs
  if ($_REQUEST['do'] != "fake_generator") {
    if (!isset($_REQUEST['id']) || !is_numeric($_REQUEST['id'])) {
      _error(400);
    }
    /* get user info */
    $user_info = $user->get_user($_REQUEST['id']);
    if (!$user_info) {
      throw new Exception(__("This account not exist"));
    }
    /* check if the user is admin and viewer is not admin */
    if ($user_info['user_group'] == '1' && !$user->_is_admin) {
      modal("MESSAGE", __("System Message"), __("You can't edit an admin user"));
    }
  }
  ...
  switch ($_REQUEST['do']) {
    case 'approve':
      /* approve user */
      $db->query(sprintf("UPDATE users SET user_approved = '1' WHERE user_id = %s", secure($user_info['user_id'], 'int')));
      /* notify the user */
      $user->post_notification(['to_user_id' => $user_info['user_id'], 'action' => 'pending_user_approved']);
      /* return */
      return_json();
      break;
    ...
    case 'edit_account':
      // check admin|moderator permission
      if (!$user->_is_admin) {
        modal("MESSAGE", __("System Message"), __("You have to be admin to edit this settings"));
      }
      ...
      if ($user_group != '1' && $_GET['id'] == '1') {
        modal("MESSAGE", __("System Message"), __("You can't change the group of this user"));
      }
      /* update */
    ...
    case 'edit_privacy':
      ...
      $_POST['user_newsletter_enabled'] = (isset($_POST['user_newsletter_enabled'])) ? '1' : '0';
      /* check if tips enabled */
      $_POST['user_tips_enabled'] = (isset($_POST['user_tips_enabled'])) ? '1' : '0';
      ...
      $_POST['user_privacy_gender'] = (isset($_POST['user_privacy_gender'])) ? $_POST['user_privacy_gender'] : 'public';
      ...
      $_POST['user_privacy_subscriptions'] = (isset($_POST['user_privacy_subscriptions'])) ? $_POST['user_privacy_subscriptions'] : 'public';
      /* check if pages enabled */
      $_POST['user_privacy_pages'] = (isset($_POST['user_privacy_pages'])) ? $_POST['user_privacy_pages'] : 'public';
      /* check if groups enabled */
      $_POST['user_privacy_groups'] = (isset($_POST['user_privacy_groups'])) ? $_POST['user_privacy_groups'] : 'public';
      /* check if events enabled */
      $_POST['user_privacy_events'] = (isset($_POST['user_privacy_events'])) ? $_POST['user_privacy_events'] : 'public';
      ...
      $db->query(sprintf(
        ...
        secure($_POST['user_newsletter_enabled']),
        secure($_POST['user_tips_enabled']),
        secure($_POST['user_privacy_chat']),
        ...
        secure($_POST['user_privacy_subscriptions']),
    ...
    case 'edit_extra':
      ...
      /* update */
      $db->query(sprintf("UPDATE users SET user_wallet_balance = %s, user_points = %s, user_monetization_enabled = %s WHERE user_id = %s", secure($_POST['user_wallet_balance']), secure($_POST['user_points']), secure($_POST['user_monetization_enabled']), secure($_GET['id'], 'int')));
      /* return */
      return_json(['success' => true, 'message' => __("User info have been updated")]);
      break;
    ...
    case 'fake_generator':
      ...
      /* return */
      return_json(['success' => true, 'message' => $generated . " " . __("fake users accounts generated successfully")]);
      break;
