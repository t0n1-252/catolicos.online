...
try {
    ...
    switch ($_GET['handle']) {
      ...
      case 'marketplace':
        ...
        if (!$payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $orders_collection['total'], 'coinbase', 'marketplace');
          /* redirect */
  ...
  // update user
  $db->query(sprintf("UPDATE users SET coinbase_hash = '', coinbase_code = '' WHERE user_id = %s", secure($user->_data['user_id'], 'int')));
